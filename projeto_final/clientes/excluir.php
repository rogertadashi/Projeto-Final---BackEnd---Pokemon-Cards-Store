...
if (!in_array($funcao, ['Administrador', 'Vendedor'], true)) {
    $_SESSION['flash'] = 'Apenas administradores e vendedores podem excluir clientes.';
...
if ($id <= 0) {
    ...
    if (!$cliente) {
        ...
        if ($del->execute()) {
            $mensagem = "Cliente <strong>" . htmlspecialchars($nomeCliente) . "</strong> excluído com sucesso!";
        } else {
            $erro = 'Erro ao excluir cliente. Tente novamente.';
        }
    }
}
?>
